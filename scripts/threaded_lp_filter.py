...
class FilterTimer():
    ...
    def timer(self, event):
        ...
        if len(self.q) == self.queue_size:
            data = np.array(self.q)
            data = fft_filter(data, l_freq_thresh=0.0001, amp_thresh=15, attenuation=0.8, cut_off_freq=10, sample_rate=250)
...
def main():
    ...
    Node(ft_rate=250.0, cutoff_freq=10.0, order=2, queue_size=500)
